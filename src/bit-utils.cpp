...
#ifndef BIT_UTILS_CPP_
#define BIT_UTILS_CPP_

#include <math.h>

#include <climits>

unsigned getNOnes(unsigned n) { return (1u << n) - 1; }

unsigned getNOnesAt(unsigned n, unsigned offset) {
  return getNOnes(n) << offset;
}

unsigned getMaxN() { return CHAR_BIT * sizeof(unsigned); }
...
unsigned setNBitsAt(unsigned value, unsigned bits, unsigned offset,
                    unsigned n) {
  const unsigned max_n = getMaxN();
